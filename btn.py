...
def act(fmp_parser_per, ms_parser_per, yahoo_parser_per, screener_parser_per, zacks_parser_per, appl_data):
    len_ap = len(appl_data)
    if fmp_parser_per:
        clear_res(2, 4, len_ap)
        fmp_parser.main(appl_data)
        with open("log.txt", "a") as log_txt:
            log_txt.write(f"FMP {date.today()}\n")

    if ms_parser_per:
        clear_res(4, 67, len_ap)
...
def main():
    ...
    if appl_checker(appl_data):
        ...
        with open("config_btn.txt") as f:
            data = f.readlines()
            screener_parser_per = data[0].strip().split("=")[1].replace('"', '').lower() == "true"
            fmp_parser_per = data[1].strip().split("=")[1].replace('"', '').lower() == "true"
            yahoo_parser_per = data[2].strip().split("=")[1].replace('"', '').lower() == "true"
            ms_parser_per = data[3].strip().split("=")[1].replace('"', '').lower() == "true"
            zacks_parser_per = data[4].strip().split("=")[1].replace('"', '').lower() == "true"

            print("Your config_btn:")
            for i in data:
                print("\t", i.strip())
        act(fmp_parser_per=fmp_parser_per,
            ms_parser_per=ms_parser_per,
